...
struct Point {
    ...
    // 1: b 在 this 左侧 | -1: b 在 this 右侧
    // 2: b 与 this 同向 | -2：b 与 this 反向
    int check(const Point& b)const {
        T c = *this ^ b;
        T d = *this * b;
        if (c > eps) return 1;
        else if (c < -eps) return -1;
        else if (abs(c) <= eps && d > eps) return 2;
        else return -2;
    }
};
...
double distance(Point p, Point a, Point b) {
    Point v1 = b - a, v2 = p - a;
    return fabs((v1 ^ v2) / sqrt(v1.len2()));
}
```
